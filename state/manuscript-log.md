...
- **2026-08-09 — PARTS I AND II EXPANDED to roughly 2.3×** on the author's instruction, by turning
 ...
 stakes** — which is why every recap line below is still true and none needed rewriting. Verified:
 zero prose-lint FAILs across all 37 chapters, no invented proper nouns against the pre-expansion
 text, em-dash rate at or near zero everywhere. **Parts III–IV are queued for the same pass**; see
 the lopsidedness warning in `state/story-bible.md` STATUS.
...
day down by the river): Soren spends the afternoon on the floor by the hearth with his slate. Emlyn
...
(BLIND — de-staged after the blind-seed probe caught the first draft telegraphing; rendered as an
ordinary hesitation, "the way a man will check his stride on a stair he has climbed a hundred times");
Jorin notices, both silent. Emlyn carries the CHEST down and sets it in the cellar's NE corner ("where
the wall stood driest" — no self-examination). A recording BLANK is set flush in the floor for a 30-day
sub-harmonic read (Deep Grammar Phase One begins → reported Ch 14). RIVER HOUSE SOLD: money straight to
principal; her things crated unsorted under Penworth's seal and stored; Emlyn signs the flags without
reading the two-page INVENTORY, files it UNREAD in the sticking drawer (grief SEALED — the callipers
seed lives in that same drawer). NEW FACTS: cellar built (geography updated); chest moved Study-NE →
cellar-NE; recording blank in cellar floor; river house sold (money mid-book, crates sealed); inventory
= new grief object, unread. CLOSING: image — the new stone lying quiet in the dark cellar, listening,
...
corner it "has always kept a little to itself, colder than cold, for reasons that have no words even in
the hill" (twist kept VEILED — the first draft decoded the NE-corner reaching-toward-the-unarrived and
was cut). And it does the new thing: it "says him back," saying Emlyn's shape into the dark along with
...
book's best question:** "So what are you *for*, then?" → his answer (he is the one "allowed to waste a
great deal of time being wrong," because Hal cannot afford to be) → her verdict: **"That's not really
